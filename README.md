...
1. Use the provided requirements.txt to install needed packages for the project. [?]
	- I am using a recommended variation.
2. Make fundamental project visibly functional enough that it can be debugged. [X]
...
3. Use django-debug-toolbar to find places where database queries run too long or hit the database too many times. [X]
...
5. Check that models are using appropriate fields for the type of data they store. If not, correct them and create migrations to handle the data. [X]
6. Check that forms are using the correct fields and validation. If not, fix. [X]
